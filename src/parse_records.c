...
int parse_record(char* input_record, s_record_t *s_record)
{
    ...
    while(state != END_STATE)
    {
        ...
        case LENGTH_STATE:
            if(*byte_pointer > MAX_RECORD_LENGTH)
            {
                /* Record too long */
                return_status = -4;
                state = END_STATE;
        ...
        case ADDRESS_STATE:
            for (int i = 1; i < ADDRESS_LENGTH; i++)
            {
                ...
                s_record->address[i - 1] = *byte_pointer;
                /* Add byte to sum and increment pointer */
                sum += *byte_pointer++;
            }
            /* Move to next state */
        ...
        case DATA_STATE:
            ...
            for (int i = ADDRESS_LENGTH; i < s_record->length - 1; i++)
            {
                ...
                s_record->data[i - ADDRESS_LENGTH] = *byte_pointer;
                /* Add byte to sum and increment pointer */
                sum += *byte_pointer++;
            }
            /* Move to next state */
